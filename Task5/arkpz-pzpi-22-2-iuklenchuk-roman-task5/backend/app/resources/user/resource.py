...
from .schemas import RentalResponseSchema, UserResponseSchema, UserUpdateSchema, LockResponseSchema, WarehouseSchema
...
@user_router.get("/my-warehouses/", response_model=list[WarehouseSchema])
async def get_user_warehouses_list(user=Depends(Authorization(allowed_roles=[UserRole.SELLER])), db=Depends(get_db)):
    user_warehouses = await get_user_warehouses(db, user)
    res = []
    for warehouse in user_warehouses:
        premium_services_query = select(PremiumService).filter(PremiumService.warehouse_id == warehouse.id)
        premium_services_result = await db.execute(premium_services_query)
        warehouse_services = premium_services_result.scalars().all()
        r = dict(
            **warehouse.__dict__,
            services=[
                    dict(
