...
@rent_router.get("/seller-rents/", response_model=list[RentalResponseSchema])
async def get_seller_rents(user=Depends(Authorization(allowed_roles=[UserRole.SELLER])), db=Depends(get_db)):
    ...
    for warehouse in warehouses:
        query = select(Rental).filter(Rental.warehouse_id == warehouse.id)
        result = await db.execute(query)
        rentals_ = result.scalars().all()
        r_ = []
        for r in rentals_:
            user = await User.get_user_by_id(db, r.user_id)
            res = dict(
                **r.__dict__,
